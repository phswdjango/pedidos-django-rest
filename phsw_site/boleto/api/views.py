...
from phsw_site.boleto.api.serializers import BoletoSerializer
from phsw_site.boleto.utils import gerar_boleto
    ...
    if request.user.all_api_permissions:
        ...
        if serializer.is_valid():
            if serializer.validated_data["comando"] == 'boleto':
                boleto = gerar_boleto(serializer.validated_data['modelo'], serializer.validated_data['campos'])
                return boleto
                # return Response({"response": "Boleto criado com sucesso."}, status=status.HTTP_200_OK)
            else:
